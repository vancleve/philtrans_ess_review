...
\end{equation}
\end{subequations}
The two conditions in \eqref{eq:ess:def} are equivalent to requiring that phenotype $\ess{x}$ receives higher average fitness than any alternative phenotype $y$ when $y$ is rare in the population \cite{Maynard-Smith:1974,Bishop:Cannings:1976}. If condition \eqref{eq:ess:def} holds as a strict inequality, then $\ess{x}$ is a ``strict'' ESS.
...
\section{Invasion and lineage fitness and ESSs}
...
While Result \ref{res:twolocus} emphasizes the role of ESSs and phenotypic evolution in the long-term process, the external stability approach used to prove \ref{res:twolocus} reveals a more general viewpoint where ESSs can depend in important ways on genetic and demographic parameters of a population. The external stability analysis assumes that the population is at some genetic equilibrium and is subject to potential invasion by a new mutant allele at low frequency and looks for a genetic equilibrium and its resulting phenotype that resists invasion by any mutant allele within a set of phenotypic effects. Before the mathematical machinery of external stability was developed, Hamilton appreciated the conceptual importance of uninvadable phenotypes in his study of the sex ratio, and he called sex ratios that resisted invasion ``unbeatable'' phenotypes \cite{Hamilton:1967}. As we saw above, determining whether a phenotype can be unbeatable and its genetic equilibria externally stable involves calculating the magnitude of the eigenvalue $\eig$ associated with the linear dynamical system that approximates the dynamics of the mutant allele frequency. Assuming that $\eig >0$, the phenotype $\ess{x}$ is unbeatable whenever $\eig(\ess{x},y) < 1$ for all $y \ne \ess{x}$ (within some set of effects).

%% a few things:
%% \item have to say \eig(x,x) = 1
%% \item have to work with strict ESS
%% \item maybe have to say this is for continuum of pure strategies?
%% \item maybe I can dodge that?
%% \item

external stability analysis assumes that new mutant allele arises in a population at low enough frequency that so that its dynamics are well approximated by a linear approximation of the genotype frequency dynamical equations. These linear dynamical systems either grow or contract geometrically depending on the magnitude of the eigenvalue $\eig$ of the (Jacobian) matrix where growth occurs for $\eig > 1$ (assuming $\eig>0$) and contraction for $\eig < 1$. Thus, rare mutant alleles will either successfully invade and reach some nonzero frequency, after which point their dynamics are no longer linear, or they will go extinct. The magnitude of the eigenvalue $\eig$ measures the geometric growth rate (e.g., equation \eqref{eq:lambda:twolocus}) of the mutant genotype relative to the resident genotypes and so is really a measure of the fitness of the invading mutant genotype and is often called ``invasion fitness'' \cite{Metz:Nisbet:1992,Heino:Metz:1998} (relatedly, ``invasion exponent'' \cite{Rand:Wilson:1994}). Regardless of the complexity of the genetics, demography, or ecology of a population, we can in principle perform an external stability analysis\footnote{This is also referred to as an ``evolutionary invasion analysis''.} that uses invasion fitness to predict the fate of rare mutants.
...
ESS phenotypes of the long-term process are produced by externally stable genetic equilibria, and the conditions for the stability of those equilibria depend on specific assumptions about the genetic system of the evolving population (e.g., random mating and autosomal loci).

A deeper aspect of how genotypic and phenotypic processes are enmeshed in Result \ref{res:twolocus} comes from looking at how external stability generates the ESS condition, or how phenotypic evolutionary stability emerges out of genotypic evolutionary stability.


Given a measure of invasion fitness, we can then look for a phenotype $x$ that cannot be invaded by any other phenotype;
Hamilton called such phenotypes ``unbeatable'' \cite{Hamilton:1967}.
An unbeatable phenotype $\ess{x}$ is one where all possible mutant phenotypes $y$ have an invasion fitness $\eig(y,\ess{x}) < 1$.
In fact, an unbeatable phenotype $\ess{x}$ is a strict ESSs \cite{Eshel:Feldman:1998}. To see this, recall that from equation \eqref{eq:lambda:twolocus} that the invasion fitness is just the ratio of the mutant fitness against the resident to the resident fitness against itself, or $\eig(y,\ess{x}) = w(y,\ess{x})/w(\ess{x}, \ess{x})$. Since $\eig < 1$ for all mutant $y$ phenotypes, this means $w(y,\ess{x}) < w(\ess{x},\ess{x})$, which is the definition of a strict ESS from equation \eqref{eq:ess:def:a}. Thus, finding an ESS $\ess{x}$ means calculating the right invasion fitness for a population and showing that $\eig(\ess{x},y) < 1$ for all possible $y$.



As we saw in section \ref{sec:long-term}, the invasion fitness $\eig$ has an associated eigenvector $v$ that measures the frequency of mutant genotypes during the invasion process.
